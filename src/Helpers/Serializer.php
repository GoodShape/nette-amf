...
class Serializer {
    ...
    protected function writeDouble($d) {
        ...
        if (Utils::isSystemBigEndian()) { // if we are a big-endian processor
            $r = strrev($b);
     ...
     */
    protected function writeXML(\stdClass $d) {
     ...
     */
    protected function writeDate(\DateTime $d) {
        $this->writeByte(0x0B);
        $this->writeDouble($d->getTimestamp());
        $this->writeInt(0);
    }
    ...
    protected function writeData($d) {
        if ($this->packet->amfVersion == Deserializer::AMF3_ENCODING) { //amf3 data. This is most often, so it's has been moved to the top to be first
            $this->writeByte(0x11);
            ...
            $this->writeNull();
            return;
        } elseif (is_object($d) && get_class($d) == 'App\\Core\\Amf\\Utils\\UndefinedType') { // undefined
            $this->writeUndefined();
            return;
        } elseif (is_array($d)) { // array
            $this->writeArrayOrObject($d);
            return;
        } elseif ($d instanceof \DateTime) { // date
            $this->writeDate($d);
            return;
        ...
        throw new \Exception("couldn't write data " . print_r($d));
    }
    ...
    protected function writeAmf3Data($d) {
        if (is_int($d)) { //int
            ...
            return;
        } elseif ($d instanceof ByteArray) { //byte array
     ...
     */
    protected function writeAmf3Date(\DateTime $d) {
        $this->writeByte(0x08);
        $this->writeAmf3Int(1);
     ...
     */
    protected function writeAmf3ByteArray(ByteArray $d) {
        $this->writeByte(0x0C);
        $data = $d->data;
